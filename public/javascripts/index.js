...
$(function() {
  ...
  $('#exec_send_nc').on('click', function() {
    $('#confirm_send_user_name').text(
      $('#send_user_list li[data-id="' + sendUserId + '"]').data('name')
    );
  ...
  $.ajax({
    ...
    .done(function(res) {
      ...
      $.each(res, function(channelId, channelData) {
        $('#send_channel').append(
          '<option value="' + channelId + '">' + channelData['name'] + '</option>'
        );
  ...
  $.ajax({
    ...
    .done(function(res) {
      ...
      $.each(res, function(i, sendData) {
        ...
        const VIEW_SEND_DATE =
          paddingValue(SEND_DATE_OBJ.getMonth() + 1, 0, 2) +
          '/' +
          paddingValue(SEND_DATE_OBJ.getDate(), 0, 2) +
          ' ' +
          paddingValue(SEND_DATE_OBJ.getHours(), 0, 2) +
          ':' +
          paddingValue(SEND_DATE_OBJ.getMinutes(), 0, 2);
        let viewUserName = '-';
        let propUserName = '';
        ...
        $('#send_nc_history_list').append(
          '<div class="send_nc_history_row"><div>' +
            VIEW_SEND_DATE +
            '</div><div>' +
            sendData['channel'] +
            '</div><div' +
...
function setHistoryListMaxHeight() {
  $('#send_nc_history_list').css(
    'max-height',
    'calc(100vh - ' +
      $('#send_nc_history_list').offset()['top'] +
      'px - ' +
      $('#send_nc_history_section').css('margin-bottom') +
      ')'
  );
}
...
function switch_exec_button_enabled() {
  if (
    sendUserId !== '' &&
    $('input[name="send_nc"]:checked').length > 0 &&
    $('#send_channel').val() !== '' &&
    $('#send_comment').val() !== ''
  ) {
    $('#exec_send_nc')
