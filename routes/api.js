		...
		(async () => {
			...
			for (let userData of userList['members']) {
				if (
					...
					&& (typeof userData['profile']['always_active'] !== 'boolean' || userData['profile']['always_active'] === false)
				) {
	...
	router.post('/send_nc', function(req, res) {
		(async () => {
			...
			const sendResult = await web.chat.postMessage({
				...
				'text'		: sprintf(':%s: <@%s> c=%s', iconStr, req['body']['send_user_id'], req['body']['send_comment']),
				'as_user'	: true,
			});
			res.send({
		...
		(async () => {
			...
			do {
				...
				const rawMessageList	= await web.conversations.history({
					...
					'latest'	: new Date(dateObj.getFullYear(), dateObj.getMonth() + 1, 0, 23, 59, 59).getTime() / 1000,
				});
				console.log(dateObj.getFullYear() + '.' + (dateObj.getMonth() + 1) + ': ', rawMessageList['messages'].length);
				++cnt;
			} while (true);
			res.send([]);
		})();
